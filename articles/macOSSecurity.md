...
#### Le System Signed Volume : un sceau d'intégrité pour l'ensemble du système de fichiers 

Big Sur continue à s'appuyer sur la séparation du *Volume Group* en *System* et *Data*, mais il rajoute un système de protection supplémentaire : SSV, pour *System Signed Volume*.
...
Il semblerait que non. Le document **Apple File System Reference** indique que pour qu'un volume soit doté d'un sceau, il faut que le rôle de ce dernier soit `APFS_VOL_ROLE_SYSTEM`, associé au répertoire racine du système. C'est d'ailleurs indiqué dans le nom : *<u>System</u> Signed Volume*.

**3/ Comment vérifier si mon système utilise le SSV ?**

Les commandes `diskutil list` et  `diskutil apfs` permettent de confirmer l'utilisation du SSV. D'ailleurs on regarde les volumes concernées :
...
La volume Système est  `disk3s1` , et le snapshot associé est `disk3s1s1`. Comme on a dit plus haut que c'est le snapshot qui est scellé, on peut lancer la commande :
...
On a ici utilisé le volume Système, pas son snapshot, c'est il est normal que le sceau soit indiqué comme *broken*. On fait la même chose pour le snapshot :
